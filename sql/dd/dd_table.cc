  ...
  */
  ...
  for (int key_nr = 1; key != end; ++key, ++key_nr) {
    ...
    */
    ...
    // storing next_spatial info
    if (key->next_spatial_index_config.type() != NEXT_SPATIAL_INDEX_TYPE::NONE) {
      idx_options->set("next_spatial_index_type",
                       (uint)key->next_spatial_index_config.type());
    }
