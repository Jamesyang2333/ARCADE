...
Item_func_semantic_rank::Item_func_semantic_rank(THD *thd, const POS &pos,
                                                   PT_item_list *a)
    : Item_func_fb_vector_distance(thd, pos, a) {}

const char *Item_func_semantic_rank::func_name() const { return "semantic_rank"; }

enum Item_func::Functype Item_func_semantic_rank::functype() const {
  return SEMANTIC_RANK;
}

bool Item_func_semantic_rank::resolve_type(THD *thd) {
  if (param_type_is_default(thd, 0, 1, MYSQL_TYPE_JSON)) return true;
  if (param_type_is_default(thd, 1, 1, MYSQL_TYPE_VARCHAR)) return true;
  set_nullable(true);
  return false;
}

double Item_func_semantic_rank::val_real() {
  if (args[0]->null_value || args[1]->null_value) {
    return error_real();
  }

  try {
    // First argument is the vector (same as existing vector functions)
    Fb_vector vector1;
    if (parse_fb_vector_from_item(args, 0, m_value, func_name(), vector1)) {
      return error_real();
    }

    // Second argument is the text to embed
    String *text_str = args[1]->val_str(&m_value);
    if (!text_str || args[1]->null_value) {
      return error_real();
    }

    // Check if we've already embedded this text
    if (!m_text_embedding_cached || m_text_cache.length() != text_str->length() ||
        memcmp(m_text_cache.ptr(), text_str->ptr(), text_str->length()) != 0) {

      // Cache the text and get its embedding
      m_text_cache.copy(*text_str);

      if (embed_text_to_vector(text_str, m_input_vector)) {
        return error_real();
      }
      m_text_embedding_cached = true;
    }
...
bool Item_func_semantic_rank::embed_text_to_vector(const String *text_str, Fb_vector &vector) {
#ifdef WITH_SEMANTICDB
  std::string text(text_str->ptr(), text_str->length());
  std::vector<float> embedding;

  if (semantic_embed_openai(text, &embedding)) {
    my_error(ER_SIGNAL_EXCEPTION, MYF(0), "Failed to embed text with OpenAI API", func_name());
    return true;
  }

  // Set the vector data
  std::vector<float> &vector_data = vector.get_data_ref();
  vector_data = std::move(embedding);
...
float Item_func_semantic_rank::compute_distance(const float *v1, const float *v2,
                                                 size_t dimension) {
  return faiss::fvec_L2sqr(v1, v2, dimension);
}
...
float Item_func_semantic_rank::compute_distance(const float *v1 [[maybe_unused]],
                                                 const float *v2 [[maybe_unused]],
                                                 size_t dimension
                                                 [[maybe_unused]]) {
  FB_VECTORDB_DISABLED_ERR;
