  ...
  for (uint i = skip_gipk ? 1 : 0; i < share->keys; i++, key_info++) {
    ...
    for (uint j = 0; j < key_info->user_defined_key_parts; j++, key_part++) {
      ...
      if (key_part->field &&
          ...
          !key_info->is_fb_vector_index() && !key_info->is_next_spatial_index()) {
...
/**
  show next spatial index options
*/
static void store_next_spatial_index_options(String *packet, KEY *key_info) {
  if (!key_info->is_next_spatial_index()) {
    return;
  }

  NEXT_spatial_index_config next_spatial_index_info = key_info->next_spatial_index_config;
  packet->append(" NEXT_SPATIAL_INDEX_TYPE '");
  auto next_spatial_index_type_str =
      next_spatial_index_type_to_string(next_spatial_index_info.type());
  packet->append(next_spatial_index_type_str);
  packet->append("'");
}
...
static void store_key_options(THD *thd, String *packet, TABLE *table,
  ...
  if (!foreign_db_mode) {
    ...
    */
    ...
    store_next_spatial_index_options(packet, key_info);
