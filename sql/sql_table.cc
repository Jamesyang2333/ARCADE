...
static bool prepare_key_column(THD *thd, HA_CREATE_INFO *create_info,
  ...
  */
  ...
  // next spatial index only support type `geometry` 
  if (key_info->is_next_spatial_index()) {
    if (sql_field->sql_type != MYSQL_TYPE_GEOMETRY) {
      my_error(ER_WRONG_ARGUMENTS, MYF(0),
               "next spatial index only support geometry type");
      return true;
    }
    if (sql_field->is_nullable) {
      my_error(ER_WRONG_ARGUMENTS, MYF(0),
               "next spatial index column should not be nullable");
      return true;
    }
  }
  ...
  */
  ...
  if (key_part_length > file->max_key_part_length(create_info) &&
      key->type != KEYTYPE_FULLTEXT && !key_info->is_fb_vector_index() && !key_info->is_next_spatial_index()) {
    key_part_length = file->max_key_part_length(create_info);
    if (key->type == KEYTYPE_MULTIPLE) {
      /* not a critical problem */
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_TOO_LONG_KEY,
...
/**
  set next spatial index info to key_info
*/
static bool prepare_next_spatial_index(const Key_spec *key, KEY *key_info) {
  if (key->key_create_info.m_next_spatial_index_type.length == 0) {
    // not a next_spatial index, do nothing and return
    return false;
  }

  if (!NEXT_SPATIALDB_ENABLED) {
    my_error(ER_FEATURE_DISABLED, MYF(0), "next spatial db", "WITH_NEXT_SPATIALDB");
    return true;
  }

  // do not allow create primary/unique/multiple index for next_spatial index
  if (key->type != KEYTYPE_SPATIAL) {
    my_error(ER_WRONG_ARGUMENTS, MYF(0),
             "next_spatial index can only be KEYTYPE_SPATIAL");
    return true;
  }
  ...
  if (key->columns.size() != 1) {
    my_error(ER_WRONG_ARGUMENTS, MYF(0),
             "next spatial index can only have one column");
    return true;
  }

  NEXT_SPATIAL_INDEX_TYPE next_spatial_index_type;
  if (parse_next_spatial_index_type(key->key_create_info.m_next_spatial_index_type,
                                 next_spatial_index_type)) {
    my_error(ER_WRONG_ARGUMENTS, MYF(0), "invalid next_spatial_index_type");
    return true;
  }
  key_info->next_spatial_index_config = NEXT_spatial_index_config(
      next_spatial_index_type);
  return false;
}
...
static bool prepare_key(
  ...
  */
  ...
  // set key info for next spatial index if used
  if (prepare_next_spatial_index(key, key_info)) {
    return true;
  }
...
// prepare next_spatial key for alter and upgrade
static void prepare_next_spatial_key(KEY *key_info,
    KEY_CREATE_INFO &key_create_info) {
  if (!key_info->is_next_spatial_index()) {
  return;
  }
  std::string_view next_spatial_index_type =
  next_spatial_index_type_to_string(key_info->next_spatial_index_config.type());
  key_create_info.m_next_spatial_index_type = LEX_CSTRING{
  .str = next_spatial_index_type.data(), .length = next_spatial_index_type.length()};
}
  ...
  */
  ...
  for (uint i = 0; i < table->s->keys; i++, key_info++) {
    ...
    if (key_parts.elements) {
      ...
      prepare_next_spatial_key(key_info, key_create_info);
      /*
...
bool mysql_alter_table(THD *thd, const char *new_db, const char *new_name,
  ...
  */
  ...
  // Check to see if the table exists
  {
    TABLE *table = table_list->table;
    if (table == NULL &&
        table_list->open_strategy == Table_ref::OPEN_IF_EXISTS) {
      /*
