...
/**
  fill next spatial index options
*/
static void fill_next_spatial_index_from_dd(MEM_ROOT *mem_root,
  const dd::Properties &idx_options,
  KEY *keyinfo) {
if (idx_options.exists("next_spatial_index_type")) {
uint next_spatial_index_type;
if (idx_options.get("next_spatial_index_type", &next_spatial_index_type)) {
assert(false);
}
keyinfo->next_spatial_index_config = NEXT_spatial_index_config(
...
static bool fill_index_from_dd(THD *thd, TABLE_SHARE *share,
  ...
  */
  ...
  fill_next_spatial_index_from_dd(&share->mem_root, idx_options, keyinfo);
