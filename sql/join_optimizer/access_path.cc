  ...
  while (!todo.empty()) {
    ...
    switch (path->type) {
      ...
      case AccessPath::INDEX_SCAN: {
        ...
        if (param.table->key_info[param.idx].is_next_spatial_index()) {
          param.table->file->next_spatial_index_init(*join->order.order->item);
        }
        break;
      }
      case AccessPath::REF: {
      ...
      case AccessPath::INDEX_RANGE_SCAN: {
        ...
        if (table->key_info[param.index].is_next_spatial_index()) {
          // table->file->next_spatial_index_init(*join->order.order->item);
        }
