...
class Rdb_next_spatial_index_scanner : public Rdb_tables_scanner {
  public:
   Rdb_next_spatial_index_scanner(my_core::THD *thd, my_core::TABLE *table)
       : m_thd(thd), m_table(table) {}
   int add_table(Rdb_tbl_def *tdef) override;
...
/*
  Support for INFORMATION_SCHEMA.ROCKSDB_NEXT_SPATIAL_INDEX dynamic table
 */
namespace RDB_NEXT_SPATIAL_INDEX_FIELD {
  enum {
    INDEX_NUMBER = 0,
    TABLE_SCHEMA,
    TABLE_NAME,
  ...
  static ST_FIELD_INFO rdb_i_s_next_spatial_index_config_fields_info[] = {
      ROCKSDB_FIELD_INFO("INDEX_NUMBER", sizeof(uint32), MYSQL_TYPE_LONG, 0),
      ROCKSDB_FIELD_INFO("TABLE_SCHEMA", NAME_LEN + 1, MYSQL_TYPE_STRING, 0),
      ROCKSDB_FIELD_INFO("TABLE_NAME", NAME_LEN + 1, MYSQL_TYPE_STRING, 0),
      ROCKSDB_FIELD_INFO("INDEX_NAME", NAME_LEN + 1, MYSQL_TYPE_STRING, 0),
  ...
  int Rdb_next_spatial_index_scanner::add_table(Rdb_tbl_def *tdef) {
    assert(tdef != nullptr);
    ...
    const std::string &dbname = tdef->base_dbname();
    field[RDB_NEXT_SPATIAL_INDEX_FIELD::TABLE_SCHEMA]->store(
        dbname.c_str(), dbname.size(), system_charset_info);
  
    const std::string &tablename = tdef->base_tablename();
    field[RDB_NEXT_SPATIAL_INDEX_FIELD::TABLE_NAME]->store(
        tablename.c_str(), tablename.size(), system_charset_info);
    ...
    for (uint i = 0; i < tdef->m_key_count; i++) {
      ...
      field[RDB_NEXT_SPATIAL_INDEX_FIELD::INDEX_NAME]->store(
          kd.m_name.c_str(), kd.m_name.size(), system_charset_info);
  
      GL_INDEX_ID gl_index_id = kd.get_gl_index_id();
      field[RDB_NEXT_SPATIAL_INDEX_FIELD::INDEX_NUMBER]->store(gl_index_id.index_id,
                                                         true);
  
      NEXT_spatial_index_config next_spatial_config = kd.get_next_spatial_index_config();
      std::string_view index_type =
          next_spatial_index_type_to_string(next_spatial_config.type());
      field[RDB_NEXT_SPATIAL_INDEX_FIELD::INDEX_TYPE]->store(
          index_type.data(), index_type.size(), system_charset_info);
      auto next_spatial_index = kd.get_next_spatial_index();
      auto next_spatial_index_info = next_spatial_index->dump_info();
      field[RDB_NEXT_SPATIAL_INDEX_FIELD::NTOTAL]->store(next_spatial_index_info.m_ntotal,
                                                   true);
  ...
  static int rdb_i_s_next_spatial_index_config_fill_table(
      my_core::THD *thd, my_core::Table_ref *tables,
      my_core::Item *cond MY_ATTRIBUTE((__unused__))) {
    DBUG_ENTER_FUNC();
    ...
    Rdb_next_spatial_index_scanner ddl_arg(thd, tables->table);
    Rdb_ddl_manager *ddl_manager = rdb_get_ddl_manager();
    assert(ddl_manager != nullptr);
  ...
  static int rdb_i_s_next_spatial_index_config_init(void *p) {
    my_core::ST_SCHEMA_TABLE *schema;
    ...
    schema->fields_info = rdb_i_s_next_spatial_index_config_fields_info;
    schema->fill_table = rdb_i_s_next_spatial_index_config_fill_table;
...
struct st_mysql_plugin rdb_i_s_next_spatial_index_config = {
  MYSQL_INFORMATION_SCHEMA_PLUGIN,
  &rdb_i_s_info,
  "ROCKSDB_NEXT_SPATIAL_INDEX",
  "JINGYI YANG",
  "next spatial index information and stats",
  PLUGIN_LICENSE_GPL,
  rdb_i_s_next_spatial_index_config_init,
  nullptr, /* uninstall */
  rdb_i_s_deinit,
  0x0002,  /* version number (0.2) */
  nullptr, /* status variables */
