...
#include <string>
#include <vector>
#include <fstream> // output to txt for testing
...
int Rdb_convert_to_record_value_decoder::decode(uchar *const buf, TABLE *table,
  ...
  if (is_null) {
    ...
    if (!field_dec->m_is_virtual_gcol) {
      ...
      } else if (field_dec->m_field_type == MYSQL_TYPE_GEOMETRY) {
        err = decode_geometry(table, ptr, field_dec, reader, decode);
      } else {
...
/*
  Convert geometry from rocksdb storage format into Mysql Record format
  @param    table       IN           current table
  @param    field       IN           current field
  @param    reader      IN           rocksdb value slice reader
  @param    decode      IN           whether to decode current field
  @return
    ...
    other  HA_ERR error code (can be SE-specific)
*/
int Rdb_convert_to_record_value_decoder::decode_geometry(
  ...
  uint32 data_len = Field_geom::get_length(
      reinterpret_cast<const uchar *>(data_len_str), length_bytes);
  const char *geom_ptr;
  if (!(geom_ptr = reader->read(data_len))) {
    return HA_ERR_ROCKSDB_CORRUPT_DATA;
  }
  ...
  if (decode) {
    ...
    memcpy(buf + length_bytes, &geom_ptr, sizeof(uchar **));

    // String geom_str(geom_ptr, data_len, &my_charset_bin);
    // Field_geom *geom_field = reinterpret_cast<Field_geom *>(table->field[field_dec->m_field_index]);
    // if (geom_field->store(geom_str.ptr(), geom_str.length(), &my_charset_bin)) {
    //   fprintf(stderr, "Error: Failed to store geometry data\n");
    //   return HA_ERR_ROCKSDB_CORRUPT_DATA;
    // }
  }
...
int Rdb_converter::encode_value_slice(
  ...
  for (uint i = 0; i < m_table->s->fields; i++) {
    ...
    if (encoder.m_field_type == MYSQL_TYPE_BLOB || //encoder.m_field_type == MYSQL_TYPE_GEOMETRY ||
    ...
    } else if (encoder.m_field_type == MYSQL_TYPE_GEOMETRY) {
      my_core::Field_geom *geom =
          reinterpret_cast<my_core::Field_geom *>(field);
      /* Get the number of bytes needed to store length*/
      const uint length_bytes = geom->pack_length() - portable_sizeof_char_ptr;

      /* Store the length of the value */
      m_storage_record.append(reinterpret_cast<char *>(geom->field_ptr()),
                              length_bytes);

      /* Store the blob value itself */
      char *data_ptr;
      memcpy(&data_ptr, geom->field_ptr() + length_bytes, sizeof(uchar **));
      m_storage_record.append(data_ptr, geom->get_length());
