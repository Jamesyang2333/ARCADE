...
class Rdb_transaction {
  ...
  */
  ...
  [[nodiscard]] virtual std::unique_ptr<rocksdb::Iterator> get_iterator_next_spatial(
    const rocksdb::ReadOptions &options,
    rocksdb::ColumnFamilyHandle &column_family, TABLE_TYPE table_type) = 0;

  [[nodiscard]] virtual std::unique_ptr<rocksdb::Iterator> get_iterator_vector(
    const rocksdb::ReadOptions &options,
    rocksdb::ColumnFamilyHandle &column_family, TABLE_TYPE table_type) = 0;
  ...
  [[nodiscard]] std::unique_ptr<rocksdb::Iterator> get_iterator_next_spatial(
      rocksdb::ColumnFamilyHandle &column_family, 
      TABLE_TYPE table_type, const std::vector<double>& query_coordinates,
      bool read_current = false, bool create_snapshot = true) {
    // Make sure we are not doing both read_current (which implies we don't
    // want a snapshot) and create_snapshot which makes sure we create
    // a snapshot
    ...
    iterator_context.query_mbr.append(reinterpret_cast<const char*>(&query_coordinates[0]), sizeof(double));
    iterator_context.query_mbr.append(reinterpret_cast<const char*>(&query_coordinates[1]), sizeof(double));
    iterator_context.query_mbr.append(reinterpret_cast<const char*>(&query_coordinates[2]), sizeof(double));
    iterator_context.query_mbr.append(reinterpret_cast<const char*>(&query_coordinates[3]), sizeof(double));
    options.iterator_context = &iterator_context;
    options.is_secondary_index_scan = true;
    ...
    return get_iterator_next_spatial(options, column_family, table_type);
  }
  
  [[nodiscard]] std::unique_ptr<rocksdb::Iterator> get_iterator_vector(
      rocksdb::ColumnFamilyHandle &column_family, 
      TABLE_TYPE table_type, std::vector<float>& query_vector, uint k, uint nprobe,
      bool read_current = false, bool create_snapshot = true) {
    // Make sure we are not doing both read_current (which implies we don't
    // want a snapshot) and create_snapshot which makes sure we create
    // a snapshot
    ...
    options.num_probes = nprobe;
    options.num_returns = k;
    options.is_ivf_vector_index_scan = true;
    options.async_io = true;
    ...
    // log_to_file(
    //     "get_iterator_vector: query_vector size: " + std::to_string(query_vector.size()) +
    //     ", k: " + std::to_string(k) + ", nprobe: " + std::to_string(nprobe));
    return get_iterator_vector(options, column_family, table_type);
  }
...
// Helper functions to extract values from JSON strings
int extract_json_int(const std::string& json, const std::string& key) {
  ...
  // Find the end of the value (comma or end of object)
  ...
  */
  ...
  for (size_t i = 0; i < cf_names.size(); ++i) {
    ...
    */
    ...
    // Retrieve field info list and table config if exists
    std::string cf_name = cf_names[i];

    std::string field_info_path = std::string(rocksdb_datadir) + "/table_options/" + 
                                cf_name + "_field_info_list.json";
    std::string table_config_path = std::string(rocksdb_datadir) + "/table_options/" + 
                                  cf_name + "_table_config.json";

    // Check if files exist
    rocksdb::Status file_status = rocksdb::Env::Default()->FileExists(field_info_path);
    if (file_status.ok()) {
      // Read field_info_list JSON file
      std::string field_info_content;
      rocksdb::Status read_status = rocksdb::ReadFileToString(
          rocksdb::Env::Default(), field_info_path, &field_info_content);
      
      if (!read_status.ok()) {
        LogPluginErrMsg(WARNING_LEVEL, ER_LOG_PRINTF_MSG,
                      "RocksDB: Failed to read field info list from %s: %s",
                      field_info_path.c_str(), read_status.ToString().c_str());
      } else {
        // Read table_config JSON file
        std::string table_config_content;
        read_status = rocksdb::ReadFileToString(
            rocksdb::Env::Default(), table_config_path, &table_config_content);
        
        if (!read_status.ok()) {
          LogPluginErrMsg(WARNING_LEVEL, ER_LOG_PRINTF_MSG,
                        "RocksDB: Failed to read table config from %s: %s",
                        table_config_path.c_str(), read_status.ToString().c_str());
        } else {
          // Parse field_info_list JSON
          std::vector<rocksdb::BlockBasedTableOptions::FieldInfo> field_info_list;
          try {
            // Parse the JSON array
            size_t pos = 0;
            while ((pos = field_info_content.find('{', pos)) != std::string::npos) {
              size_t end_pos = field_info_content.find('}', pos);
              if (end_pos == std::string::npos) break;
              
              std::string obj = field_info_content.substr(pos, end_pos - pos + 1);
              rocksdb::BlockBasedTableOptions::FieldInfo field_info;
              
              // Extract each field
              field_info.type = extract_json_int(obj, "type");
              field_info.is_nullable = extract_json_bool(obj, "is_nullable");
              field_info.null_mask = extract_json_int(obj, "null_mask");
              field_info.length_bytes = extract_json_int(obj, "length_bytes");
              field_info.pack_length = extract_json_int(obj, "pack_length");
              
              field_info_list.push_back(field_info);
              pos = end_pos + 1;
            }
            
            // Parse table_config JSON
            rocksdb::BlockBasedTableOptions::TableConfig table_config;
            table_config.null_bytes_length = extract_json_int(table_config_content, "null_bytes_length");
            table_config.has_ttl = extract_json_bool(table_config_content, "has_ttl");
            table_config.has_unpack_info = extract_json_bool(table_config_content, "has_unpack_info");
            table_config.has_checksums = extract_json_bool(table_config_content, "has_checksums");
            
            // Set the field info list and table config in the options
            auto block_based_table_factory = 
              dynamic_cast<rocksdb::BlockBasedTableFactory *>(opts.table_factory.get());
            if (block_based_table_factory && field_info_list.size() > 0) {
              // Create a new table factory that's a clone of the original
              auto new_options = block_based_table_factory->table_options();
              auto new_factory = std::make_shared<rocksdb::BlockBasedTableFactory>(new_options);
              auto new_block_factory = dynamic_cast<rocksdb::BlockBasedTableFactory *>(new_factory.get());
              
              // Modify the NEW factory
              new_block_factory->UpdateDecodeInfo(table_config, field_info_list);
              // Set the index options if not the system column family
              // log_to_file("rocksdb_init_internal: Setting index options for cf: " + cf_name);
              if (cf_name != "__system__") {
                new_block_factory->SetIndexOptions();
              }
              
              // Replace the factory in the options
              opts.table_factory = new_factory;

              // std::cout << "Loaded decode info for cf: " << cf_name << std::endl;
              // std::cout << "With Field info list size: " << field_info_list.size() << std::endl;
            }
            
            LogPluginErrMsg(INFORMATION_LEVEL, ER_LOG_PRINTF_MSG,
                          "RocksDB: Successfully loaded decode info for %s",
                          cf_name.c_str());
          } catch (const std::exception& e) {
            LogPluginErrMsg(ERROR_LEVEL, ER_LOG_PRINTF_MSG,
                          "RocksDB: Error parsing decode information for %s: %s",
                          cf_name.c_str(), e.what());
          }
        }
      }
    }
...
int get_table_info(std::string cf_name, std::vector<rocksdb::BlockBasedTableOptions::FieldInfo>* field_info_list,
                          rocksdb::BlockBasedTableOptions::TableConfig *table_config) {
  // Retrieve field info list and table config if exists
    // log_to_file("cf_name: " + cf_name);
    ...
    // log_to_file("field_info_path: " + field_info_path);
    // log_to_file("table_config_path: " + table_config_path);
      ...
      if (!read_status.ok()) {
        ...
        if (!read_status.ok()) {
          ...
          try {
            ...
            size_t pos = 0;
            while ((pos = field_info_content.find('{', pos)) != std::string::npos) {
              // log_to_file("Found '{' at position: " + std::to_string(pos));
              size_t end_pos = field_info_content.find('}', pos);
              if (end_pos == std::string::npos) break;
...
int ha_rocksdb::alloc_key_buffers(const TABLE &table_arg,
  ...
  // log_to_file("setup pk key_def in ha_rocksdb::alloc_key_buffers");
  ...
  for (uint i = 0; i < table_arg.s->keys; i++) {
    ...
    // log_to_file("setup sk key_def in ha_rocksdb::alloc_key_buffers");
...
/**
  Helper function to determine if a table will require unpack info during key creation.
  
  This function analyzes the table structure to infer whether unpack_info will be needed
  based on the primary key fields and their characteristics. Unpack info is typically 
  needed when:
  1. The primary key contains variable length fields (VARCHAR, BLOB, JSON)
  2. Fields cannot be fully encoded within their key parts
  
  @param[in]  table_arg   Table structure containing field and key information
  @return     bool        True if unpack info is likely needed, False otherwise
  
  @note       This is an approximation - the actual unpack info requirement is 
              determined during key definition creation. This function provides 
              an early inference that may help with optimization decisions.
*/
bool will_need_unpack_info(const TABLE &table_arg) {
  // Get primary key index
  uint pk_index = 0;  // Default to first index if no explicit PK
  for (uint i = 0; i < table_arg.s->keys; i++) {
  ...
  // Look at primary key fields
  KEY *pk_info = &table_arg.key_info[pk_index];
  for (uint i = 0; i < pk_info->user_defined_key_parts; i++) {
    Field *field = pk_info->key_part[i].field;
    
    // Variable length fields in PK typically need unpack info
    ...
    // Fields that can't be fully encoded in key also need unpack info 
    if (field->pack_length() > pk_info->key_part[i].length) {
      return true;
    }
  }
...
bool ha_rocksdb::create_cfs(
  ...
  // Get the decode information from table_arg
  rocksdb::BlockBasedTableOptions::TableConfig table_config;
  table_config.null_bytes_length = 
      table_arg.s->null_bytes;
      
  table_config.has_ttl = 
      table_arg.s->fields > 0 && 
      table_arg.field[0]->real_type() == MYSQL_TYPE_TIMESTAMP;
      
  table_config.has_unpack_info = will_need_unpack_info(table_arg);
      
  table_config.has_checksums = 
      should_store_row_debug_checksums();

  // bool contain_spatial_column = false;
  // for (uint i = 0; i < table_arg.s->fields; i++) {
  //   Field *field = table_arg.field[i];
  //   if (field->real_type() == MYSQL_TYPE_GEOMETRY) {
  //     contain_spatial_column = true;
  //     break;
  //   }
  // }
  // log_to_file("Table contain spatial column: " + std::to_string(contain_spatial_column));

  // Then populate field info list
  std::vector<rocksdb::BlockBasedTableOptions::FieldInfo> field_info_list;
  field_info_list.clear();
  for (uint i = 0; i < table_arg.s->fields; i++) {
    Field *field = table_arg.field[i];

    // Skip if field is part of primary key
    bool is_pk_field = false;
    if (table_arg.s->primary_key != MAX_KEY) {
      KEY *pk_info = &table_arg.s->key_info[table_arg.s->primary_key];
      for (uint j = 0; j < pk_info->user_defined_key_parts; j++) {
        if (i == pk_info->key_part[j].fieldnr - 1) {
          is_pk_field = true;
          break;
        }
      }
    ...
    rocksdb::BlockBasedTableOptions::FieldInfo field_info;
    field_info.type = field->real_type();
    field_info.is_nullable = field->is_nullable();
    field_info.null_mask = 1 << field->null_offset();

    // Set length bytes for variable length fields
    if (field->type() == MYSQL_TYPE_VARCHAR) {
      // Length bytes is 1 for small VARCHARs (<= 255), 2 for longer ones
      field_info.length_bytes = field->field_length > 255 ? 2 : 1;
    } else if (field->type() == MYSQL_TYPE_BLOB || 
              field->type() == MYSQL_TYPE_JSON ||
              field->type() == MYSQL_TYPE_GEOMETRY) {
      // Set length bytes based on BLOB/JSON type
      switch (field->real_type()) {
        case MYSQL_TYPE_TINY_BLOB:
          field_info.length_bytes = 1;
          break;
        case MYSQL_TYPE_BLOB:
          field_info.length_bytes = 2;
          break;
        case MYSQL_TYPE_MEDIUM_BLOB:
          field_info.length_bytes = 3;
          break;
        case MYSQL_TYPE_LONG_BLOB:
          field_info.length_bytes = 4;
          break;
        default:
          // JSON uses LONGBLOB storage
          field_info.length_bytes = 4;
          break;
      }
    }

    // Set pack length for fixed length fields  
    if (field->type() != MYSQL_TYPE_VARCHAR &&
        field->type() != MYSQL_TYPE_BLOB &&
        field->type() != MYSQL_TYPE_JSON &&
        field->type() != MYSQL_TYPE_GEOMETRY) {
      field_info.pack_length = field->pack_length();
    }
  ...
  */
  std::set<std::string> updated_cf_names;
  for (uint i = 0; i < tbl_def_arg.m_key_count; i++) {
    ...
    {
      // Again, need a more robust check and proper naming meahcnism
      // if (!field_info_list.empty()) {
      //   cf_name.append("_decode");
      // }
      ...
      if (updated_cf_names.find(cf_name) == updated_cf_names.end()) {
        // We need a more robust check here to see if table requires decode info for building index
        if (!field_info_list.empty()) {
          // Update the decode information
          auto cf_options = rdb->GetOptions(cf_handle.get());
          auto table_factory = cf_options.table_factory;
          auto block_based_table_factory =
              dynamic_cast<rocksdb::BlockBasedTableFactory *>(table_factory.get());
          if (block_based_table_factory) {
            // Create a new table factory that's a clone of the original
            // auto new_options = block_based_table_factory->table_options();
            // auto new_factory = std::make_shared<rocksdb::BlockBasedTableFactory>(new_options);
            // auto new_block_factory = dynamic_cast<rocksdb::BlockBasedTableFactory *>(new_factory.get());
            
            // // Update decode info in new factory
            // new_block_factory->UpdateDecodeInfo(table_config, field_info_list);
            // // Replace the factory in the options
            // cf_options.table_factory = new_factory;
            block_based_table_factory->UpdateDecodeInfo(table_config, field_info_list);
            // log_to_file("create_cfs: Setting index options for cf: " + cf_name);
            block_based_table_factory->SetIndexOptions();
            // if (contain_spatial_column) {
            //   log_to_file("Enable enableSecondaryIndex in block based table option");
            //   block_based_table_factory->EnableSecondaryIndex();
            // }
          }

          // Persist the decode information to disk in JSON format
          std::string field_info_list_json = cf_name + "_field_info_list.json";
          std::string table_config_json = cf_name + "_table_config.json";

          // Create table_options directory if it doesn't exist
          std::string table_options_dir = std::string(rocksdb_datadir) + "/table_options";
          // std::cout << "Writing decode information to: " << table_options_dir << std::endl;
          rocksdb::Status s = rocksdb::Env::Default()->CreateDirIfMissing(table_options_dir);
          if (!s.ok()) {
            // NO_LINT_DEBUG
            LogPluginErrMsg(ERROR_LEVEL, ER_LOG_PRINTF_MSG,
                            "RocksDB: Failed to create table_options directory: %s",
                            s.ToString().c_str());
            return true; // Return error
          }

          // Write field_info_list to JSON file
          std::string field_info_path = table_options_dir + "/" + field_info_list_json;
          std::ofstream field_info_file(field_info_path);
          if (!field_info_file.is_open()) {
            // NO_LINT_DEBUG
            LogPluginErrMsg(ERROR_LEVEL, ER_LOG_PRINTF_MSG,
                            "RocksDB: Failed to open file for writing: %s",
                            field_info_path.c_str());
            return true; // Return error
          }

          field_info_file << "[\n";
          for (size_t i = 0; i < field_info_list.size(); i++) {
            const auto& field_info = field_info_list[i];
            field_info_file << "  {\n";
            field_info_file << "    \"type\": " << field_info.type << ",\n";
            field_info_file << "    \"is_nullable\": " << (field_info.is_nullable ? "true" : "false") << ",\n";
            field_info_file << "    \"null_mask\": " << field_info.null_mask << ",\n";
            field_info_file << "    \"length_bytes\": " << field_info.length_bytes << ",\n";
            field_info_file << "    \"pack_length\": " << field_info.pack_length << "\n";
            field_info_file << "  }" << (i < field_info_list.size() - 1 ? "," : "") << "\n";
          }
          field_info_file << "]\n";
          field_info_file.close();

          // Write table_config to JSON file
          std::string table_config_path = table_options_dir + "/" + table_config_json;
          std::ofstream table_config_file(table_config_path);
          if (!table_config_file.is_open()) {
            // NO_LINT_DEBUG
            LogPluginErrMsg(ERROR_LEVEL, ER_LOG_PRINTF_MSG,
                            "RocksDB: Failed to open file for writing: %s",
                            table_config_path.c_str());
            return true; // Return error
          }

          table_config_file << "{\n";
          table_config_file << "  \"null_bytes_length\": " << table_config.null_bytes_length << ",\n";
          table_config_file << "  \"has_ttl\": " << (table_config.has_ttl ? "true" : "false") << ",\n";
          table_config_file << "  \"has_unpack_info\": " << (table_config.has_unpack_info ? "true" : "false") << ",\n";
          table_config_file << "  \"has_checksums\": " << (table_config.has_checksums ? "true" : "false") << "\n";
          table_config_file << "}\n";
          table_config_file.close();
          updated_cf_names.insert(cf_name);
        }
      }
...
uint ha_rocksdb::create_inplace_key_defs(
  ...
  for (i = 0; i < tbl_def_arg.m_key_count; i++) {
    ...
    // log_to_file("setup key_def in ha_rocksdb::create_inplace_key_def");
...
int ha_rocksdb::create_key_def(
  ...
  // initialize key_def
  // log_to_file("setup key_def in ha_rocksdb::create_key_def");
...
int ha_rocksdb::secondary_index_read(const int keyno, uchar *const buf,
  ...
  if (covered_lookup && m_lock_rows == RDB_LOCK_NONE) {
    ...
    // std::string s(reinterpret_cast<const char*>(buf), 47);
    // log_to_file("buff after get_row_by_rowid: ");
    // log_to_file_raw(s);
...
int ha_rocksdb::secondary_index_parse(const int keyno, uchar *const buf,
                                     const rocksdb::Slice *key,
                                     const rocksdb::Slice *value,
                                     bool *skip_row, std::string &record_value) {
  DBUG_TRACE;
  assert(table != nullptr);
  ...
  if (covered_lookup && m_lock_rows == RDB_LOCK_NONE) {
    ...
    DEBUG_SYNC(ha_thd(), "rocksdb_concurrent_upd_or_delete_sk");
    rc = get_row_from_value(buf, m_last_rowkey.ptr(), m_last_rowkey.length(), record_value,
                          skip_row, false, !rdb_is_binlog_ttl_enabled());
    // std::string s(reinterpret_cast<const char*>(buf), 47);
    // log_to_file("buff after get_row_from_value: ");
    // log_to_file_raw(s);
    if (skip_row && *skip_row) goto done;
...
int ha_rocksdb::index_read_intern(uchar *const buf, const uchar *const key,
  ...
  if (kd.is_vector_index()) {
    // log_to_file("index_read_intern is_vector_index()");
    ...
    if (kd.get_vector_index_config().type() == FB_VECTOR_INDEX_TYPE::LSMIDX) {
      size = vector_index_key.size();
      std::memcpy(m_pk_packed_tuple, vector_index_key.data(), vector_index_key.size());
      if (size == RDB_INVALID_KEY_LEN) {
        // log_to_file("rocksdb corrupt data");
        rc = HA_ERR_ROCKSDB_CORRUPT_DATA;
        DBUG_RETURN(rc);
      }
    } else {
      // log_to_file("vector_index_key: ");
      // log_to_file_raw(vector_index_key);
      size =
    ...
    // log_to_file("size: " + std::to_string(size) + " primary key: ");
    // std::string pk(reinterpret_cast<const char*>(m_pk_packed_tuple), size);
    // log_to_file_raw(pk);
    ...
    if (kd.get_vector_index_config().type() == FB_VECTOR_INDEX_TYPE::LSMIDX) {
      ...
      int rc1 = vector_db_handler->current_value(current_value);
      if (rc1) {
        DBUG_RETURN(rc1);
      }
      rc = secondary_index_parse(active_index, buf, &key, &value, &skip_row, current_value);
      DBUG_RETURN(rc);
    } else {
      rc = secondary_index_read(active_index, buf, &key, &value, &skip_row);
      DBUG_RETURN(rc);
  ...
  if (kd.is_next_spatial_index()) {
    double x_min;
    double x_max;
    double y_min;
    double y_max;
    memcpy(&y_min, key, sizeof(double));
    memcpy(&y_max, key + 8, sizeof(double));
    memcpy(&x_min, key + 16, sizeof(double));
    memcpy(&x_max, key + 24, sizeof(double));
    // log_to_file("Parsing start key range");
    // log_to_file(std::to_string(x_min)+" "+std::to_string(x_max)+" "+std::to_string(y_min)+" "+std::to_string(y_max));
    auto next_spatial_db_handler = get_next_spatial_db_handler();
    // TODO_JC
    rc = next_spatial_db_handler->range_search(thd,
        kd.get_next_spatial_index(), x_min, x_max, y_min, y_max);
    if (rc) {
      DBUG_RETURN(rc);
    }
    if (!next_spatial_db_handler->has_more_results()) {
      DBUG_RETURN(HA_ERR_END_OF_FILE);
    }
    // const auto &pk =
    //     next_spatial_db_handler->current_pk(m_pk_descr->get_index_number());
    // log_to_file("get_row_by_rowid");
    // rc = get_row_by_rowid(buf, pk.data(), pk.size(), nullptr, false, false);
    // DBUG_RETURN(rc);
    std::string next_spatial_index_key;
    rc = next_spatial_db_handler->current_key(next_spatial_index_key);
    if (rc) {
      DBUG_RETURN(rc);
    }
    
    // log_to_file("next_spatial_index_key: ");
    // log_to_file_raw(next_spatial_index_key);
    rocksdb::Slice key(next_spatial_index_key);
    // const uint size =
    //     kd.get_primary_key_tuple(*m_pk_descr, &key, m_pk_packed_tuple);
    const uint size = next_spatial_index_key.size();
    std::memcpy(m_pk_packed_tuple, next_spatial_index_key.data(), next_spatial_index_key.size());
    if (size == RDB_INVALID_KEY_LEN) {
      // log_to_file("rocksdb corrupt data");
      rc = HA_ERR_ROCKSDB_CORRUPT_DATA;
      DBUG_RETURN(rc);
    ...
    // log_to_file("size: " + std::to_string(size) + " primary key: ");
    // std::string pk(reinterpret_cast<const char*>(m_pk_packed_tuple), size);
    // log_to_file(std::to_string(m_pk_packed_tuple[0]));
    // log_to_file(std::to_string(m_pk_packed_tuple[4]));
    // log_to_file_raw(pk);
    bool skip_row = false;
    rocksdb::Slice value;
    // rc = secondary_index_read(active_index, buf, &key, &value, &skip_row);
    ...
    std::string current_value;
    int rc1 = next_spatial_db_handler->current_value(current_value);
    if (rc1) {
      DBUG_RETURN(rc1);
    }
    rc = secondary_index_parse(active_index, buf, &key, &value, &skip_row, current_value);
    // std::string s(reinterpret_cast<const char*>(buf), current_value.size());
    // log_to_file("current_value size: " + std::to_string(current_value.size()));
    // log_to_file("buff: ");
    // log_to_file_raw(s);
    // log_to_file("current_value: ");
    // log_to_file_raw(current_value);
    DBUG_RETURN(rc);
  }
...
int ha_rocksdb::index_read_map(uchar *const buf, const uchar *const key,
                               ...
                               enum ha_rkey_function find_flag) {
  DBUG_ENTER_FUNC();
  // log_to_file("ha_rocksdb::index_read_map");
...
// int ha_rocksdb::read_range_first(const key_range *start_key, const key_range *end_key,
//   bool eq_range, bool sorted) {
//   DBUG_ENTER_FUNC();
//   log_to_file("entering ha_rocksdb::read_range_first");

//   int rc = 0;
//   THD *thd = ha_thd();
//   const Rdb_key_def &kd = *m_key_descr_arr[active_index_pos()];
  
//   // Parse bounding box from key buffer
//   const uchar *ptr = start_key->key;
//   double min_x = float8get(ptr); ptr += 8;
//   double max_x = float8get(ptr); ptr += 8;
//   double min_y = float8get(ptr); ptr += 8;
//   double max_y = float8get(ptr);             // done

//   log_to_file("Parsing start key range");
//   log_to_file(std::to_string(min_x)+" "+std::to_string(max_x)+" "+std::to_string(min_y)+" "+std::to_string(max_y));

//   if (kd.is_next_spatial_index()) {
//     auto next_spatial_db_handler = get_next_spatial_db_handler();
//     rc = next_spatial_db_handler->range_search(thd,
//         kd.get_next_spatial_index());
...
//         next_spatial_db_handler->current_pk(m_pk_descr->get_index_number());
//     // rc = get_row_by_rowid(buf, pk.data(), pk.size(), nullptr, false, false);
//     memcpy(table->record[0], 0, table->s->reclength);
//     DBUG_RETURN(0);
//   }
// }

// int ha_rocksdb::read_range_next(){
//   DBUG_ENTER_FUNC();
//   log_to_file("entering ha_rocksdb::read_range_next");
...
//       DBUG_RETURN(HA_ERR_END_OF_FILE);
//     }
//     next_spatial_db_handler->next_result();
//     const auto &pk =
//         next_spatial_db_handler->current_pk(m_pk_descr->get_index_number());
//     // rc = get_row_by_rowid(buf, pk.data(), pk.size(), nullptr, false, false);
//     memcpy(table->record[0], 0, table->s->reclength);
...
Rdb_next_spatial_db_handler *ha_rocksdb::get_next_spatial_db_handler() {
  if (m_next_spatial_db_handler == nullptr) {
    m_next_spatial_db_handler = std::make_unique<Rdb_next_spatial_db_handler>();
  }
  return m_next_spatial_db_handler.get();
}
...
int ha_rocksdb::get_row_by_rowid(uchar *const buf, const char *const rowid,
  ...
  if (!rc) {
    m_last_rowkey.copy((const char *)rowid, rowid_size, &my_charset_bin);
    // log_to_file("key_slice and m_retrieved_record in get_row_by_rowid: ");
    // log_to_file_raw(key_slice.ToString());
    // log_to_file_raw(m_retrieved_record.ToString());
...
int ha_rocksdb::get_row_from_value(uchar *const buf, const char *const rowid,
                                 const uint rowid_size, std::string &value, bool *skip_row,
                                 const bool skip_lookup,
                                 const bool skip_ttl_check) {
  DBUG_ENTER_FUNC();
  ...
  if (m_insert_with_update && m_dup_key_found &&
    ...
    // rc = get_pk_iterator()->get(&key_slice, &m_retrieved_record, m_lock_rows,
    //                             skip_ttl_check, skip_wait);
    // log_to_file("directly copy value to m_retrieved_record");
    m_retrieved_record.PinSelf(rocksdb::Slice(value));
    rc = 0;
  }

  if (!rc) {
    m_last_rowkey.copy((const char *)rowid, rowid_size, &my_charset_bin);
    // log_to_file("key_slice and m_retrieved_record in get_row_from_value: ");
    // log_to_file_raw(key_slice.ToString());
    // log_to_file_raw(m_retrieved_record.ToString());
    rc = convert_record_from_storage_format(&key_slice, buf);
...
/**
  @return
    ...
    other            HA_ERR error code (can be SE-specific)
*/
int ha_rocksdb::next_spatial_index_init(Item *distance_func) {
  auto next_spatial_db_handler = get_next_spatial_db_handler();
  return next_spatial_db_handler->next_spatial_index_init(distance_func, 100);
  return HA_EXIT_SUCCESS;
}
...
void ha_rocksdb::next_spatial_index_end() {
  const Rdb_key_def &kd = *m_key_descr_arr[active_index_pos()];
...
static int calculate_stats(
  ...
  if (scan_type == SCAN_TYPE_FULL_TABLE && thd) {
    for (auto &key : to_recalc) {
      ...
      // if (key.second->is_next_spatial_index()) {
      //   ret = key.second->get_next_spatial_index()->analyze(thd, max_num_rows_scanned,
      //                                                 killed);
      //   if (ret) {
      //     DBUG_RETURN(ret);
      //   }
...
//TODO_JC
std::unique_ptr<rocksdb::Iterator> rdb_tx_get_iterator_next_spatial(
  THD *thd, rocksdb::ColumnFamilyHandle &cf,
  const rocksdb::Snapshot **snapshot, TABLE_TYPE table_type, 
  const std::vector<double>& query_coordinates,
  bool read_current, bool create_snapshot) {
  // log_to_file("rdb_tx_get_iterator_next_spatial");
  if (commit_in_the_middle(thd)) {
    // log_to_file("read from snapshot");
    assert(snapshot && *snapshot == nullptr);
    if (snapshot) {
      // log_to_file("snapshot exists");
      *snapshot = rdb->GetSnapshot();
      auto read_opts = rocksdb::ReadOptions();
      rocksdb::RtreeIteratorContext iterator_context;
      iterator_context.query_mbr.append(reinterpret_cast<const char*>(&query_coordinates[0]), sizeof(double));
      ...
      iterator_context.query_mbr.append(reinterpret_cast<const char*>(&query_coordinates[2]), sizeof(double));
      iterator_context.query_mbr.append(reinterpret_cast<const char*>(&query_coordinates[3]), sizeof(double));
      read_opts.iterator_context = &iterator_context;
      read_opts.is_secondary_index_scan = true;
      ...
      // log_to_file("snapshot not exist");
      return nullptr;
    }
  } else {
    // log_to_file("get iterator from transaction");
    Rdb_transaction *tx = get_tx_from_thd(thd);
    return tx->get_iterator_next_spatial(cf, table_type, query_coordinates, read_current,
                            create_snapshot);
    }
}

std::unique_ptr<rocksdb::Iterator> rdb_tx_get_iterator_vector(
  THD *thd, rocksdb::ColumnFamilyHandle &cf,
  const rocksdb::Snapshot **snapshot, TABLE_TYPE table_type, 
  std::vector<float>& query_vector, uint k, uint nprobe,
  bool read_current, bool create_snapshot) {
  // log_to_file("rdb_tx_get_iterator_next_spatial");
  if (commit_in_the_middle(thd)) {
    // log_to_file("read from snapshot");
    ...
    if (snapshot) {
      ...
      *snapshot = rdb->GetSnapshot();
      auto read_opts = rocksdb::ReadOptions();
      read_opts.query_vector = &query_vector;
      read_opts.num_probes = nprobe;
      read_opts.num_returns = k;
      read_opts.is_ivf_vector_index_scan = true;
      read_opts.async_io = true;
      // TODO(mung): set based on WHERE conditions
      // read_opts.total_order_seek = true;
      read_opts.snapshot = *snapshot;
    ...
    // log_to_file("get iterator from transaction");
    Rdb_transaction *tx = get_tx_from_thd(thd);
    return tx->get_iterator_vector(cf, table_type, query_vector, k, nprobe, read_current,
                            create_snapshot);
  }
}
...
ha_rows ha_rocksdb::multi_range_read_info_const(uint keyno, RANGE_SEQ_IF *seq,
  ...
  // if (res == HA_POS_ERROR) {
  //   log_to_file("res == HA_POS_ERROR after calling handler::multi_range_read_info_const");
  // }
  if (res == HA_POS_ERROR) {
...
bool ha_rocksdb::index_supports_vector_scan(ORDER *order, int idx) {
  ...
  if ((strcmp(item_func->func_name(), "+") == 0) && (((Item_func *)item_func)->argument_count() == 2)) {
    Item *arg0 = (Item *)item_func->arguments()[0];
    Item *real_arg0 = arg0->real_item();
    if (real_arg0->type() == Item::FUNC_ITEM) {
      item_func = (Item_func *)real_arg0;
    } else {
      return false;
    }
  }
...
mysql_declare_plugin(rocksdb_se){
    ...
    myrocks::rdb_i_s_vector_index_config,
    myrocks::rdb_i_s_next_spatial_index_config mysql_declare_plugin_end;
