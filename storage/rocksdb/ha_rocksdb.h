...
class ha_rocksdb : public my_core::handler, public blob_buffer {
  ...
  */
  ...
  /*
    store state for next spatial db operations
  */
  std::unique_ptr<Rdb_next_spatial_db_handler> m_next_spatial_db_handler;
  ...
  */
  ...
  int secondary_index_parse(const int keyno, uchar *const buf,
                           const rocksdb::Slice *key,
                           const rocksdb::Slice *value, bool *skip_row,
  ...
  int get_row_from_value(uchar *const buf, const char *const rowid,
                       const uint rowid_size, std::string &value, bool *skip_row = nullptr,
                       const bool skip_lookup = false,
                       const bool skip_ttl_check = true)
      MY_ATTRIBUTE((__warn_unused_result__));
  int get_row_from_value(uchar *const buf, const uchar *const rowid,
                       const uint rowid_size, std::string &value, bool *skip_row = nullptr,
                       const bool skip_lookup = false,
                       const bool skip_ttl_check = true)
      MY_ATTRIBUTE((__warn_unused_result__)) {
    return get_row_from_value(buf, reinterpret_cast<const char *>(rowid),
                            rowid_size, value, skip_row, skip_lookup, skip_ttl_check);
  }
  ...
  */
  ulonglong table_flags() const override {
    ...
    */
    ...
    DBUG_RETURN(HA_CAN_GEOMETRY | HA_CAN_RTREEKEYS | HA_SUPPORTS_GEOGRAPHIC_GEOMETRY_COLUMN |
 ...
 private:
  ...
  Rdb_next_spatial_db_handler *get_next_spatial_db_handler();
 ...
 public:
  ...
  int next_spatial_index_init(Item *distance_func);
...
[[nodiscard]] std::unique_ptr<rocksdb::Iterator> rdb_tx_get_iterator_next_spatial(
    THD *thd, rocksdb::ColumnFamilyHandle &cf, 
    const rocksdb::Snapshot **snapshot, TABLE_TYPE table_type, 
    const std::vector<double> &query_coordinates,
    bool read_current = false, bool create_snapshot = true);

[[nodiscard]] std::unique_ptr<rocksdb::Iterator> rdb_tx_get_iterator_vector(
    THD *thd, rocksdb::ColumnFamilyHandle &cf, 
    const rocksdb::Snapshot **snapshot, TABLE_TYPE table_type, 
