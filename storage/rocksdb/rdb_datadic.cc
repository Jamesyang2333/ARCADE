...
Rdb_key_def::Rdb_key_def(const Rdb_key_def &k)
    : m_index_number(k.get_index_number()),
      ...
      m_next_spatial_index_config(k.m_next_spatial_index_config),
  ...
  */
  ...
  if (!m_maxlength) {
    ...
    if (!is_hidden_pk) {
      ...
      m_next_spatial_index_config = key_info->next_spatial_index_config;
    } else {
    ...
    // if ((key_info->flags & HA_SPATIAL) &&  key_info->algorithm == HA_KEY_ALG_RTREE) {
    //   log_to_file("key_info->flags and key_info->algorithm successfully set in rdb_datadic");
    // }

    // "unique" secondary keys support:
    m_key_parts = is_hidden_pk ? 1 : key_info->actual_key_parts;
    // log_to_file("Rdb_key_def::setup m_key_parts: ");
    // log_to_file(std::to_string(m_key_parts));
    ...
    // log_to_file("m_key_parts after adding m_pk_key_parts: ");
    // log_to_file(std::to_string(m_key_parts));
    ...
    */
    ...
    if (is_hidden_pk) {
      ...
      for (uint src_i = 0; src_i < m_key_parts; src_i++, keypart_to_set++) {
        ...
        // log_to_file(std::to_string(field->real_type()));
    ...
    // log_to_file("dst_i: ");
    // log_to_file(std::to_string(dst_i));
    ...
    // log_to_file("Rdb_key_def::setup trying to setup_vector_index");
    uint rtn = setup_vector_index(tbl, tbl_def, cmd_srv_helper);
    if (rtn) {
      // log_to_file("failed to setup vector index");
      RDB_MUTEX_UNLOCK_CHECK(m_mutex);
      return rtn;
    }
    // log_to_file("Rdb_key_def::setup successful setup_vector_index");

    // log_to_file("Rdb_key_def::setup trying to setup_next_spatial_index");
    rtn = setup_next_spatial_index(tbl, tbl_def, cmd_srv_helper);
    if (rtn) {
      // log_to_file("failed to setup next spatial index");
      RDB_MUTEX_UNLOCK_CHECK(m_mutex);
      return rtn;
    }
    // log_to_file("Rdb_key_def::setup successful setup_next_spatial_index");
...
uint Rdb_key_def::get_primary_key_tuple(const Rdb_key_def &pk_descr,
  ...
  // log_to_file("INDEX_NUMBER_SIZE: " + std::to_string(INDEX_NUMBER_SIZE));
  ...
  // log_to_file("m_key_parts: "+std::to_string(m_key_parts));
  for (i = 0; i < m_key_parts; i++) {
    // log_to_file("i = "+std::to_string(i));
    if ((pk_key_part = m_pk_part_no[i]) != -1) {
      // log_to_file("setting start_offs[pk_key_part]");
      start_offs[pk_key_part] = reader.get_current_ptr();
      // log_to_file("start_offs[i]: ");
      // std::ostringstream oss;
      // oss << static_cast<const void*>(start_offs[0]);
      // log_to_file_raw(oss.str());
    }

    if (read_memcmp_key_part(&reader, i) > 0) {
      // log_to_file("read_memcmp_key_part(&reader, i) > 0");
      continue;
      return RDB_INVALID_KEY_LEN;
    }

    if (pk_key_part != -1) {
      // log_to_file("setting end_offs[pk_key_part]");
      end_offs[pk_key_part] = reader.get_current_ptr();
      // log_to_file("end_offs[i]: ");
      // std::ostringstream oss1;
      // oss1 << static_cast<const void*>(end_offs[0]);
      // log_to_file_raw(oss1.str());
    }
  }

  for (i = 0; i < m_pk_key_parts; i++) {
    const uint part_size = end_offs[i] - start_offs[i];
    // log_to_file("part_size: " + std::to_string(part_size));
...
int Rdb_key_def::skip_max_length(const Rdb_field_packing *const fpi,
  ...
  // log_to_file("skip_max_length");
  // log_to_file("m_max_image_len: "+std::to_string(fpi->m_max_image_len));
...
int Rdb_key_def::skip_variable_length_encoding(
    ...
    Rdb_string_reader *const reader) {
  // log_to_file("skip_variable_length_encoding");
  const uchar *ptr;
  bool finished = false;
...
static void pack_vector(Rdb_field_packing *const fpi [[maybe_unused]],
  ...
  // If the vector index is LSMIDX, we don't store the vector index entry
  if (pack_ctx->vector_index->get_config().type() ==
      FB_VECTOR_INDEX_TYPE::LSMIDX) {
      // log_to_file("Vector index is LSMIDX, not storing vector entry");
      return ;
  }
...
uint Rdb_key_def::setup_next_spatial_index(const TABLE &tbl,
    const Rdb_tbl_def &tbl_def,
    Rdb_cmd_srv_helper &cmd_srv_helper) {
  if (m_next_spatial_index_config.type() == NEXT_SPATIAL_INDEX_TYPE::NONE) {
    return HA_EXIT_SUCCESS;
  }

  // the upper layer should make sure these conditions are met,
  // but we still check them here in case there are new use case
  // that does not set up the next spatial index config properly in the
  // KEY object.
  if (is_primary_key()) {
    LogPluginErrMsg(ERROR_LEVEL, ER_LOG_PRINTF_MSG,
                    "next spatial index is not supported on primary key");
    assert(false);
    return HA_ERR_UNSUPPORTED;
  }
  if (tbl_def.get_table_type() != TABLE_TYPE::USER_TABLE) {
    LogPluginErrMsg(ERROR_LEVEL, ER_LOG_PRINTF_MSG,
                    "next spatial index is only supported on user tables");
    assert(false);
    return HA_ERR_UNSUPPORTED;
  }
  // do not support ttl or any other index flags for now
  if (m_index_flags_bitmap != 0) {
    LogPluginErrMsg(INFORMATION_LEVEL, ER_LOG_PRINTF_MSG,
                    "next spatial index is not supported on ttl tables");
    return HA_ERR_UNSUPPORTED;
  }
  KEY *key_info = &tbl.key_info[m_keyno];
  // if (key_info->actual_key_parts != 1) {
  //   log_to_file(std::to_string(key_info->actual_key_parts), "/home/jingyi/Desktop/myrocks-spatial-x-db-runtime/usr/local/mysql/data/next_debug_log");
  //   LogPluginErrMsg(ERROR_LEVEL, ER_LOG_PRINTF_MSG,
  //                   "next spatial index only supports one key part");
  //   assert(false);
  //   return HA_ERR_UNSUPPORTED;
  // }
  // log_to_file("field type of next spatial index", "/home/jingyi/Desktop/myrocks-spatial-x-db-runtime/usr/local/mysql/data/next_debug_log");
  // log_to_file(std::to_string(MYSQL_TYPE_GEOMETRY), "/home/jingyi/Desktop/myrocks-spatial-x-db-runtime/usr/local/mysql/data/next_debug_log");
  // log_to_file(std::to_string(key_info->key_part[0].field->real_type()), "/home/jingyi/Desktop/myrocks-spatial-x-db-runtime/usr/local/mysql/data/next_debug_log");
  // log_to_file(std::to_string(key_info->key_part[0].field->real_type() == MYSQL_TYPE_GEOMETRY), "/home/jingyi/Desktop/myrocks-spatial-x-db-runtime/usr/local/mysql/data/next_debug_log");
  // log_to_file(std::to_string(key_info->key_part[0].field->real_type() != MYSQL_TYPE_GEOMETRY), "/home/jingyi/Desktop/myrocks-spatial-x-db-runtime/usr/local/mysql/data/next_debug_log");
  // if (!(key_info->key_part[0].field->real_type() == MYSQL_TYPE_GEOMETRY)) {
  //   log_to_file("enter error", "/home/jingyi/Desktop/myrocks-spatial-x-db-runtime/usr/local/mysql/data/next_debug_log");
  //   log_to_file(std::to_string(MYSQL_TYPE_GEOMETRY), "/home/jingyi/Desktop/myrocks-spatial-x-db-runtime/usr/local/mysql/data/next_debug_log");
  //   log_to_file(std::to_string(key_info->key_part[0].field->real_type()), "/home/jingyi/Desktop/myrocks-spatial-x-db-runtime/usr/local/mysql/data/next_debug_log");
  //   log_to_file(std::to_string(key_info->key_part[0].field->real_type() == MYSQL_TYPE_GEOMETRY), "/home/jingyi/Desktop/myrocks-spatial-x-db-runtime/usr/local/mysql/data/next_debug_log");
  //   log_to_file(std::to_string(key_info->key_part[0].field->real_type() != MYSQL_TYPE_GEOMETRY), "/home/jingyi/Desktop/myrocks-spatial-x-db-runtime/usr/local/mysql/data/next_debug_log");
  //   LogPluginErrMsg(ERROR_LEVEL, ER_LOG_PRINTF_MSG,
  //                   "next spatial index only supports geometry field");
  //   assert(false);
  //   return HA_ERR_UNSUPPORTED;
  // }
  return create_next_spatial_index(m_next_spatial_index_config, m_cf_handle, m_index_number,
...
bool Rdb_field_packing::setup(const Rdb_key_def *const key_descr,
  ...
  switch (type) {
    ...
    */
    ...
    case MYSQL_TYPE_LONG_BLOB:
    case MYSQL_TYPE_BLOB: {
      // log_to_file("rdb_datadic setup MYSQL_TYPE_BLOB");
      // log_to_file(ToString(key_descr->get_vector_index_config().type()));
    ...
    case MYSQL_TYPE_JSON:
      // log_to_file("rdb_datadic setup MYSQL_TYPE_JSON");
      // log_to_file(ToString(key_descr->get_vector_index_config().type()));
      if (!key_descr->is_vector_index()) {
        SHIP_ASSERT(
    ...
    case MYSQL_TYPE_GEOMETRY: 
      // log_to_file("rdb_datadic setup MYSQL_TYPE_GEOMETRY");
      // log_to_file(ToString(key_descr->get_next_spatial_index_config().type()));
      if (!key_descr->is_next_spatial_index()) {
        SHIP_ASSERT(
          !"Unexpected MYSQL_TYPE_GEOMETRY seen in packing for none next spatial index");
        return false;
      }
      m_pack_func = Rdb_key_def::pack_geom;
      m_max_image_len = 8;
      // m_pack_func = Rdb_key_def::pack_with_varlength_encoding;
      break;  // handling below
