...
class Rdb_key_def {
  ...
  */
  ...
  bool is_next_spatial_index() const {
    return m_next_spatial_index_config.type() != NEXT_SPATIAL_INDEX_TYPE::NONE;
  }

  NEXT_spatial_index_config get_next_spatial_index_config() const {
    return m_next_spatial_index_config;
  }

  Rdb_next_spatial_index *get_next_spatial_index() const { return m_next_spatial_index.get(); }
 ...
 private:
  ...
  NEXT_spatial_index_config m_next_spatial_index_config{};

  std::unique_ptr<Rdb_next_spatial_index> m_next_spatial_index;

  [[nodiscard]] uint setup_next_spatial_index(const TABLE &tbl,
    const Rdb_tbl_def &tbl_def,
    Rdb_cmd_srv_helper &cmd_srv_helper);
