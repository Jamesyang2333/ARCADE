/*
  ...
  //        : m_dimension{dimension}, m_k{k}, m_coordinate_vector(coordinate_vector) {
  ...
  //    uint m_k;
  //    const std::vector<float> &m_query_coordinate;
  ...
  //  class Rdb_next_spatial_range_computer {
  //   public:
  //    Rdb_next_spatial_range_computer(const std::vector<double> &query_coordinate, const float distance,
  //                            const uint batch_size)
  //        : m_distance{distance}, m_query_coordinate(query_coordinate) {

  //      m_id_pk_map.reserve(2*batch_size);
  //      m_id_pk_map_buf.reserve(2*batch_size);
  //      m_batch_id_pk_map.reserve(batch_size);
  //      // start the id with a none zero value, to make it easier to discover id
  //      // assigning and id updating bugs.
  //      m_current_id = m_id_offset = 1024;
  ...
  //     read vector from iterator and update the knn result
  //    */
  //    uint update_range_result(Rdb_next_spatial_iterator &next_spatial_iter) {
  //      auto iter_rtn = next_spatial_iter.get_pk_and_codes(m_current_pk, m_vector_codes);
  //      if (iter_rtn) {
  //        return iter_rtn;
  //      }
  //      m_batch_id_pk_map.emplace(m_current_id, m_current_pk);
  //      m_current_id++;
  //      return compute_range(false);
  //    }
   
  //    uint finalize() {
  //      if (m_vector_codes.size() > 0) {
  //        return compute_range(true);
  //      }
  //      return HA_EXIT_SUCCESS;
  //    }
   
  //    uint populate_result(std::vector<std::string> &result) {
  //      result.clear();
  ...
  //   private:
  //    float m_distance;
  //    const std::vector<double> &m_query_coordinate;
  //    std::vector<float> m_vector_codes;
  //    decltype(m_vector_codes.size()) m_codes_batch_size;
  //    std::string m_current_pk;
  ...
  //    uint compute_range(const bool finalize) {
  //      // do not compute when we haven't reached batch size
  //      if (!finalize && m_vector_codes.size() < m_codes_batch_size) {
  //        return HA_EXIT_SUCCESS;
  //      }
  ...
  //        // copy the merge result to be merged next time,
  //        // dont' need to do that if it's the last batch
  //        memcpy(m_previous_result_vals, m_merge_result_vals, 100 * sizeof(float));
  //        memcpy(m_previous_result_ids, m_merge_result_ids,
  //               100 * sizeof(faiss::idx_t));
  //      }
  ...
  //  };
   ...
   class Rdb_next_spatial_index_base : public Rdb_next_spatial_index {
    public:
     Rdb_next_spatial_index_base(const NEXT_spatial_index_config index_def,
                           std::shared_ptr<rocksdb::ColumnFamilyHandle> cf_handle,
                           const Index_id index_id)
         : m_index_id{index_id}, m_index_def{index_def}, m_cf_handle{cf_handle} {}
   
     virtual ~Rdb_next_spatial_index_base() override = default;
   
     uint add_spatial_entry(rocksdb::WriteBatchBase *write_batch,
                     const rocksdb::Slice &pk, std::vector<float> &value,
                     const rocksdb::Slice &old_pk,
                     std::vector<float> &old_value) {
    ...
    protected:
     Index_id m_index_id;
     NEXT_spatial_index_config m_index_def;
     std::shared_ptr<rocksdb::ColumnFamilyHandle> m_cf_handle;
     std::atomic<uint> m_hit{0};
   
     // the number of vectors in the inverted list
     virtual uint64 get_list_id(const std::vector<double> &value) const = 0;
   };
   
   /**
    stores vector codes in a flat list
   */
   class Rdb_next_spatial_index_global : public Rdb_next_spatial_index_base {
    public:
     Rdb_next_spatial_index_global(const NEXT_spatial_index_config index_def,
                           std::shared_ptr<rocksdb::ColumnFamilyHandle> cf_handle,
                           const Index_id index_id)
         : Rdb_next_spatial_index_base(index_def, cf_handle, index_id) {
       assert(index_def.type() == NEXT_SPATIAL_INDEX_TYPE::GLOBAL_INDEX);
     }
   
     virtual ~Rdb_next_spatial_index_global() override = default;
    ...
    uint range_search(THD *thd, std::vector<double> &query_coordinate,
            Rdb_next_spatial_range_search_params &params,
            std::vector<std::pair<std::string, std::string>> &result) {

      // log_to_file("range_search");
      ...
      // Rdb_next_spatial_iterator next_spatial_iter(thd, m_index_id, *(m_cf_handle.get()),
      //                         get_list_id(query_coordinate));
      // uint rtn;
      // Rdb_next_spatial_range_computer computer(query_coordinate, params.m_distance,
      //                         batch_size);
      // int64_t keys_scanned = 0;
      // while (next_spatial_iter.is_available()) {
      // rtn = computer.update_range_result(next_spatial_iter);
      // keys_scanned++;
      // if (rtn) {
      // return rtn;
      // }
      // next_spatial_iter.next();
      // }
      // m_ntotal = keys_scanned;
      // rtn = computer.finalize();
      // if (rtn) {
      ...
      result.clear();
      int count = 0;
      for (next_spatial_iter.seek_to_first(); next_spatial_iter.is_available(); next_spatial_iter.next()) {
        count++;
        // log_to_file("iterator results: ");
        // log_to_file("key size: " + std::to_string(next_spatial_iter.return_key_str().size()) + 
        //             ", value size: " + std::to_string(next_spatial_iter.return_val_str().size()));
        // log_to_file_raw(next_spatial_iter.return_key_str());
        // log_to_file_raw(next_spatial_iter.return_val_str());
        if (next_spatial_iter.return_val_str().size() != 0) {
          result.emplace_back(next_spatial_iter.return_key_str(), next_spatial_iter.return_val_str());
        }
      };
      // log_to_file("number of records from iterator: " + std::to_string(count));
      // log_to_file("size of iterator result: " + std::to_string(result.size()));

      return HA_EXIT_SUCCESS;
      }
   
     Rdb_next_spatial_index_info dump_info() override {
       return {.m_ntotal = m_ntotal, .m_hit = m_hit};
     }
   
    protected:
     // always return the same list id
     uint64 get_list_id(const std::vector<double> &value) const override {
   ...
   uint create_next_spatial_index(const NEXT_spatial_index_config index_def,
     ...
     if (index_def.type() == NEXT_SPATIAL_INDEX_TYPE::GLOBAL_INDEX) {
       index =
           std::make_unique<Rdb_next_spatial_index_global>(index_def, cf_handle, index_id);
       return HA_EXIT_SUCCESS;
     }
     assert(false);
     return HA_ERR_UNSUPPORTED;
   ...
   // dummy implementation for non-next_spatialdb builds
   uint create_next_spatial_index(const NEXT_spatial_index_config index_def
                            [[maybe_unused]],
                            std::shared_ptr<rocksdb::ColumnFamilyHandle> cf_handle
                            [[maybe_unused]],
  ...
  //  TODO_JC: Iterator
   ...
   Rdb_next_spatial_db_handler::Rdb_next_spatial_db_handler() {}
   
   uint Rdb_next_spatial_db_handler::decode_value_to_buffer(
       Field *field, std::vector<float> &buffer) {
     Field_geom *field_geom = down_cast<Field_geom *>(field);
     if (field_geom == nullptr) {
      LogPluginErrMsg(ERROR_LEVEL, ER_LOG_PRINTF_MSG,
  ...
  // TODO_JC
   ...
   uint Rdb_next_spatial_db_handler::range_search(THD *thd, Rdb_next_spatial_index *index, double x_min, double x_max, double y_min, double y_max) {
    // log_to_file("rdb_next_spaital_db_handler::range_search");
    m_search_result.clear();
    m_next_spatial_db_result_iter = m_search_result.cend();
  
    // if (!m_buffer.size() || !m_limit) return HA_ERR_END_OF_FILE;

    std::vector<double> query_mbr;
    query_mbr.emplace_back(x_min);
    query_mbr.emplace_back(x_max);
    query_mbr.emplace_back(y_min);
    query_mbr.emplace_back(y_max);
    ...
    Rdb_next_spatial_range_search_params params{
        .m_distance = m_distance, .m_batch_size = m_batch_size};
    uint rtn = index->range_search(thd, query_mbr, params, m_search_result);
    // log_to_file("result pk list after range search: ");
    // for (const auto& pair : m_search_result) {
    //     log_to_file("key: ");
    //     log_to_file_raw(pair.first);
    //     log_to_file("value: ");
    //     log_to_file_raw(pair.second);
    // }
    if (rtn) {
      return rtn;
    }
    m_next_spatial_db_result_iter = m_search_result.cbegin();
  
    return rtn;
  }
   
   std::string Rdb_next_spatial_db_handler::current_pk(
       const Index_id pk_index_id) const {
     Rdb_string_writer writer;
     writer.write_index_id(pk_index_id);
     writer.write_string(m_next_spatial_db_result_iter->first);
     return writer.to_slice().ToString();
   }

   uint Rdb_next_spatial_db_handler::current_key(std::string &key) const {
      key = m_next_spatial_db_result_iter->first;
      return HA_EXIT_SUCCESS;
  }

    uint Rdb_next_spatial_db_handler::current_value(std::string &value) const {
      value = m_next_spatial_db_result_iter->second;
      return HA_EXIT_SUCCESS;
  }
